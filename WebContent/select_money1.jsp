...
%>
<h2>회원 매출정보 목록</h2>
<div style="margin: 0 10%">총 <%=cnt%>건의 매출정보가 있습니다.</div>
<hr>
...
int no=0;
try{
	String sql="select custno,saleno,pcost,amount,price,pcode,to_char(sdate,'YYYY-MM-DD') from money0713 order by pcode asc,saleno desc";
	pstmt=conn.prepareStatement(sql);
	rs=pstmt.executeQuery();
	while(rs.next()){
		...
		switch(custname){
		case "100001": custname="김행복";
		break;
		case "100002": custname="이축복";
		break;
		case "100003": custname="장믿음";
		break;
		case "100004": custname="최사랑";
		break;
		case "100005": custname="진평화";
		break;
		case "100006": custname="차공단";
		break;
		}
